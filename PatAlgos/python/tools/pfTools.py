...
def reconfigurePF2PATTaus(process,
   ...
   if tauType=='shrinkingConePFTau':
       ...
       ### Important! Check how this can be done without sequences!###
...
def switchToPFMET(process,input=cms.InputTag('pfMET'), type1=False, postfix=""):
    ...
    #getattr(process,'pfMETPFlow').jets = cms.InputTag("pfJetsPFlow")
    if( not type1 ):
...
def switchToPFJets(process, input=cms.InputTag('pfNoTauClones'), algo='AK5', postfix = "", jetCorrections=('AK5PFchs', ['L1FastJet','L2Relative', 'L3Absolute']), type1=False, outputModules=['out']):
    ...
    for corr in inputJetCorrLabel[1]:
		...
		print "searching something"
			...
			###no patJetCorrFactorskt6GenJetsPFlow. Do we need them?###
...
def usePF2PAT(process,runPF2PAT=True, jetAlgo='ak5', runOnMC=True, postfix="", jetCorrections=('AK5PFchs', ['L1FastJet','L2Relative','L3Absolute'],'None'), pvCollection=cms.InputTag('offlinePrimaryVertices'), typeIMetCorrections=False, outputModules=['out']):
	...
	#pfNoJetsClones._Labelable__label='pfNoJetsClones'
	#pfNoTauClones._Labelable__label='pfNoTauClones'
	...
	#patPF2PATSequence._Labelable__label='patPF2PATSequence'
	...
	loadWithPostFix(process,"CommonTools.ParticleFlow.PF2PAT_cff",postfix)
        ...
        #patPF2PATSequence = cms.Sequence( process.patDefaultSequence )
	loadWithPostFix(process,'PhysicsTools.PatAlgos.patSequences_cff',postfix)
	...
	#loadWithPostFix(process,'CommonTools.ParticleFlow.PF2PAT_cff','')
    ...
    sequence = cms.Sequence(getattr(process,'countPatElectrons'+postfix)+getattr(process,'countPatMuons'+postfix)+getattr(process,'countPatTaus'+postfix)+getattr(process,'countPatLeptons'+postfix)+getattr(process,'countPatPhotons'+postfix)+getattr(process,'countPatJets'+postfix))
    setattr(process,'countPatCandidates'+postfix,sequence)	
