...
def adaptPFElectrons(process,module, postfix):
    ...
    ### Not needed in unscheduled mode###		
...
def usePF2PAT(process,runPF2PAT=True, jetAlgo='ak5', runOnMC=True, postfix="", jetCorrections=('AK5PFchs', ['L1FastJet','L2Relative','L3Absolute'],'Type-1'), pvCollection=cms.InputTag('offlinePrimaryVertices'), typeIMetCorrections=True, outputModules=['out']):
...
class RemoveCleaning(ConfigToolBase):
	...
	counterList = ['countPatElectrons'+postfix,'countPatMuons'+postfix,'countPatTaus'+postfix,'countPatLeptons'+postfix,'countPatPhotons'+postfix,'countPatJets'+postfix]

        ## adapt single object counters
        for m in process.producerNames().split(' '):
	    if m in counterList:
		    print m
		    if hasattr(getattr(process,m), 'src'): m.src = m.src.value().replace('cleanPat','selectedPat')
